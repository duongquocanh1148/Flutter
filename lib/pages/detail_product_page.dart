    ...
    if(snapshot.exists){        
      if(!snapshot.child(uid).hasChild('cartItems')){
          List<CartItem>? cartItems = List.empty(growable: true);
              CartItem cartItem = CartItem(
                ...
                try {
                  await FirebaseDatabase.instance
                      ...
                      .then((value) {
                        print("new");
                    nextScreen(context, CartPage(cart: cart, food: widget.food,));
                  });
              ...
              int totalPrice = 0;
              int totalQuantity = 0;
              for(int j=0; j<snapshot.child(uid).child('cartItems').children.length;j++) {
              ...
              for(int j=0; j<snapshot.child(uid).child('cartItems').children.length;j++){
                  ...
                  if(q.child('food').child('foodKey').value.toString() == widget.food.foodKey){
                    print("trung");
                    ...
                    Map<String, dynamic> map = cart.toMap();
                    cart = Cart(totalPrice: 0, totalQuantity: 0, cartItems: null, userID: uid);
                    try{
                      await FirebaseDatabase.instance.ref('carts').child(uid).set(map).then((value) => {
                        nextScreen(context, CartPage(cart: cart, food: widget.food,))
                      });
